...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        self.network = nn.Sequential(
            layer_init(nn.Conv2d(3, 32, 3, stride=2, padding=1)),
            nn.LayerNorm([32, 32, 32], eps=1e-3),
            nn.SiLU(),
            layer_init(nn.Conv2d(32, 64, 3, stride=2, padding=1)),
            nn.LayerNorm([64, 16, 16], eps=1e-3),
            nn.SiLU(),
            layer_init(nn.Conv2d(64, 128, 3, stride=2, padding=1)),
            nn.LayerNorm([128, 8, 8], eps=1e-3),
            nn.SiLU(),
            layer_init(nn.Conv2d(128, 256, 3, stride=2, padding=1)),
            nn.LayerNorm([256, 4, 4], eps=1e-3),
            nn.SiLU(),
            nn.Flatten(),
