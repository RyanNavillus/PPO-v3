...
poetry run python -m cleanrl_utils.benchmark \
    --env-ids $env_id \
    --command "poetry run python ppo_v3/ppo_envpool_tricks.py --exp-name ppo_envpool_tricks_all_notwohot_nocriticema --symlog True --percentile-scale True --unimix 0.01 --critic-zero-init True --total-timesteps $timesteps --track" \
    --num-seeds $seeds \
    --workers $workers
...
poetry run python -m openrlbenchmark.rlops \
    ...
    "ppo_envpool_tricks_notwohot_nocriticema?tag=${gittag}" \
