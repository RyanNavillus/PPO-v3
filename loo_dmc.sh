...
#`        --slurm-template-path ppov3.slurm_template
    ...
    python3 -m poetry run python3 -m cleanrl_utils.benchmark \
        --env-ids $env_id \
        --command "python ppo_v3/ppo_envpool_tricks_dmc.py --exp-name ppo_envpool_tricks_FINAL_loo_symlog --num-envs 128 --two-hot True --two-hot-range 1000 --percentile-scale True --percentile-ema-rate 0.999 --critic-ema True --critic-zero-init True --total-timesteps $timesteps --track" \
        --start-seed $startseed \
        --num-seeds 1 \
        --workers $workers \
        --slurm-gpus-per-task 1 \
